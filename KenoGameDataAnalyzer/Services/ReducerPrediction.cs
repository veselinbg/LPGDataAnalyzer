...
{
    public class GasTempCorrection
    {
        ...
        )
        {
            ...
            var errorsByRange = ordered
                .Select((d, i) =>
                {
                    ...
                    return new
                    {
                        TempRange = GetTempRange(d.Temp_GAS),
                        Error = totalError
                    };
                })
            ...
            foreach (var (range, oldValue) in currentCorrections)
            {
                ...
                {
                    result[range] = (oldValue + error - baseError).Round();
                }
                else
                {
                    result[range] = (oldValue - baseError).Round();
                }
            }
            return result;
        }
        ...
        private static string GetTempRange(double tempGas)
        {
            return Settings.GasTemperatureRanges
                .First(r => tempGas >= r.Min && tempGas <= r.Max)
                .Label;
        }
